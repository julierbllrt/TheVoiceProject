...
function recupSujetFromDB (results){
	if(!results.rows.item(0).picto.includes("Verbes") && results.rows.item(0).genre!=null && results.rows.item(0).personne!=null){
		var leSujet = {mot:results.rows.item(0).mot,genre:results.rows.item(0).genre, personne:results.rows.item(0).personne};
	}
...
function recupVerbeFromDB (results){
...
function conjugaisonPresent(sujet, verbe){
...
function conjugaisonFutur(sujet, verbe){
...
function conjugaisonPasse(sujet, verbe){
...
// </test>
	...
	if(sujet.personne!=null || sujet.genre!==null){
		...
		resultat = verbe.auxiliaire+" "+participe;
	}
...
function conjugaison(tx, results){
	var previousPicto = results.rows.item(0);
}
